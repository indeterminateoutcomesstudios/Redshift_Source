...
function run_byond_tests {
    ...
    run_test_ci "check globals build" "python tools/GenerateGlobalVarAccess/gen_globals.py redshift.dme code/_helpers/global_access.dm"
    run_test "check globals unchanged" "md5sum -c - <<< 'a3bfcbd4e5e35726b1ba5c6dec7c5e09 *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH redshift.dme"
    run_test "check no warnings in build" "grep ', 0 warnings' build_log.txt"
    run_test "run unit tests" "DreamDaemon redshift.dmb -invisible -trusted -core 2>&1 | tee log.txt"
